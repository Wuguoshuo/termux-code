"""
Content Generator for Xiaohongshu Viral Generator
...
class ContentGenerator:
    ...
    HASHTAG_TEMPLATES = {
        ...
        '护肤心得': ['#护肤心得', '#护肤', '#好物分享', '#变美', '#护肤好物'],
        '穿搭分享': ['#穿搭分享', '#穿搭', '#OOTD', '#今日穿搭', '#时尚穿搭'],
        '美食探店': ['#美食探店', '#美食', '#探店', '#美食分享', '#吃货'],
        '旅行攻略': ['#旅行攻略', '#旅行', '#旅游', '#出行', '#旅游攻略']
    }
    ...
    def generate_image_prompt(self, title: str, topic: str, style: str = 'modern minimalist') -> str:
        """
        Generate AI image prompt based on title and topic.
        ...
        style_prompts = {
            'modern minimalist': f"Clean, modern minimalist lifestyle image for social media. {title}. White or light background, professional product photography style, soft natural lighting, high quality, Xiaohongshu aesthetic",
            'cute and fresh': f"Cute and fresh lifestyle image for social media. {title}. Vibrant colors, cheerful mood, pastel color palette, soft lighting, kawaii aesthetic, high quality, Xiaohongshu style",
            'warm vintage': f"Warm vintage lifestyle image for social media. {title}. Nostalgic feel, soft warm colors, film grain texture, cozy atmosphere, retro aesthetic, high quality",
            'tech blue': f"Modern tech lifestyle image for social media. {title}. Blue color palette, clean professional look, modern technology aesthetic, sleek design, high quality",
            'nature green': f"Nature and fresh lifestyle image for social media. {title}. Green color palette, natural lighting, fresh and peaceful atmosphere, organic feel, high quality"
        }
        
        prompt = style_prompts.get(style, style_prompts['modern minimalist'])
        return prompt
    ...
    def generate_complete_post(self, topic: str, style: str = 'mixed') -> Dict[str, Any]:
        ...
        closing_style = random.choice(['interactive', 'challenge', 'teaser'])
        ...
        image_style = random.choice(['modern minimalist', 'cute and fresh', 'warm vintage'])
        image_prompt = self.generate_image_prompt(main_title, topic, image_style)
    ...
    def _generate_context(self, topic: str) -> Dict[str, str]:
        ...
        topic_contexts = {
            '职场效率': {
                'problem': '工作效率低', 'solution': '提升效率', 'method': '高效工作法',
                'result': '效率翻倍', 'identity': '职场人', 'action': '提升效率',
                'expert': '职场导师', 'famous': '职场大神', 'brand': '世界500强',
                'celebrity': '职场精英', 'time': '1年前', 'number': str(random.randint(3, 7))
            },
            '生活美学': {
                'problem': '生活太枯燥', 'solution': '打造美学生活', 'method': '生活美学技巧',
                'result': '生活更有品质', 'identity': '生活家', 'action': '提升生活品质',
                'expert': '美学专家', 'famous': '生活博主', 'brand': '无印良品',
                'celebrity': '文艺青年', 'time': '半年前', 'number': str(random.randint(5, 10))
            },
            '学习成长': {
                'problem': '学习没方法', 'solution': '高效学习', 'method': '学习技巧',
                'result': '成绩提升', 'identity': '学习者', 'action': '高效学习',
                'expert': '学习专家', 'famous': '学霸', 'brand': '知名高校',
                'celebrity': '高考状元', 'time': '3个月前', 'number': str(random.randint(3, 5))
            },
            '时间管理': {
                'problem': '时间不够用', 'solution': '管理时间', 'method': '时间管理术',
                'result': '时间自由', 'identity': '忙碌族', 'action': '管理时间',
                'expert': '时间管理大师', 'famous': '效率达人', 'brand': '知名咨询公司',
                'celebrity': '成功人士', 'time': '2个月前', 'number': str(random.randint(7, 21))
            },
            '个人成长': {
                'problem': '成长遇瓶颈', 'solution': '突破自我', 'method': '成长方法论',
                'result': '脱胎换骨', 'identity': '追求者', 'action': '突破自我',
                'expert': '成长教练', 'famous': '成功学大师', 'brand': '知名培训机构',
                'celebrity': '行业领袖', 'time': '1年前', 'number': str(random.randint(30, 90))
            },
            '护肤心得': {
                'problem': '皮肤状态差', 'solution': '改善肤质', 'method': '护肤 routine',
                'result': '皮肤变好', 'identity': '护肤达人', 'action': '科学护肤',
                'expert': '皮肤科医生', 'famous': '护肤博主', 'brand': '雅诗兰黛',
                'celebrity': '明星同款', 'time': '1年前', 'number': str(random.randint(21, 60))
            },
            '穿搭分享': {
                'problem': '穿搭没灵感', 'solution': '提升衣品', 'method': '穿搭技巧',
                'result': '衣品提升', 'identity': '时尚达人', 'action': '学习穿搭',
                'expert': '造型师', 'famous': '时尚博主', 'brand': 'ZARA',
                'celebrity': '穿搭博主', 'time': '半年前', 'number': str(random.randint(5, 15))
            },
            '美食探店': {
                'problem': '不知道吃什么', 'solution': '发现美食', 'method': '探店攻略',
                'result': '吃遍美食', 'identity': '美食家', 'action': '探店打卡',
                'expert': '美食评论家', 'famous': '美食博主', 'brand': '米其林',
                'celebrity': '吃货达人', 'time': '最近', 'number': str(random.randint(3, 10))
            },
            '旅行攻略': {
                'problem': '旅行不会规划', 'solution': '轻松出行', 'method': '旅行攻略',
                'result': '完美旅程', 'identity': '旅行者', 'action': '制定攻略',
                'expert': '旅行达人', 'famous': '旅游博主', 'brand': '携程',
                'celebrity': '旅行博主', 'time': '上个月', 'number': str(random.randint(3, 7))
            }
        }
        ...
        if topic in topic_contexts:
            ...
            context = {
                'problem': '遇到困难', 'solution': '解决问题', 'method': '实用方法',
                'result': '达成目标', 'identity': '普通人', 'action': '采取行动',
                'expert': '专业人士', 'famous': '行业专家', 'brand': '知名品牌',
                'celebrity': '知名人士', 'time': '最近', 'number': str(random.randint(3, 7))
            }
