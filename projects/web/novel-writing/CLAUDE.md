...
| **F. 剧本改编** | 将小说改编为短剧剧本 | Skill 10 → Skill 11 → Skill 12 → Skill 13 → Skill 14 → Skill 15 |
...
### ✅ 剧本改编 Skills（新增）
- **Skill 10**: `script-skills/10-analyze-novel.md` - 小说分析
- **Skill 11**: `script-skills/11-structure-analysis.md` - 结构分析
- **Skill 12**: `script-skills/12-character-adaptation.md` - 人物改编
- **Skill 13**: `script-skills/13-scene-outline.md` - 场景大纲
- **Skill 14**: `script-skills/14-write-script.md` - 剧本创作
- **Skill 15**: `script-skills/15-script-review.md` - 剧本审校
...
判断任务类型
   ...
   └─ F. 剧本改编 ⭐ 新增
        ↓
        判断改编类型？
        ├─ 完整改编 → Skill 10 → Skill 11 → Skill 12 → Skill 13（所有集数）→ Skill 14 → Skill 15
        └─ 继续改编某集 → Skill 13（单集场景大纲）→ Skill 14 → Skill 15
```
...
├── skills/              # 小说写作 Skills 模板
│   ├── 03-discuss-chapter.md  ✅ 已实现
│   ├── 07-write-draft.md      ✅ 已实现
│   └── 08-three-pass-review.md ✅ 已实现
├── script-skills/       # 剧本改编 Skills 模板（新增）
│   ├── 10-analyze-novel.md       ✅ 已实现
│   ├── 11-structure-analysis.md  ✅ 已实现
│   ├── 12-character-adaptation.md ✅ 已实现
│   ├── 13-scene-outline.md      ✅ 已实现
│   ├── 14-write-script.md        ✅ 已实现
│   └── 15-script-review.md       ✅ 已实现
├── _scripts/            # 剧本改编工作区（新增）
│   ├── _briefs/         # 剧本设定文档
│   ├── adaptations/     # 改编计划
│   ├── drafts/          # 剧本草稿
...
```
用户：写第1集剧本
AI: [触发 Skill 13] 设计第1集场景大纲...
    等待用户确认场景大纲...
    [触发 Skill 14] 创作第1集完整剧本...
    [触发 Skill 15] 审校第1集剧本...
```
...
5. **用户确认** - 重要决策必须等待确认
6. **剧本改编流程** - 新增6个Skills，系统化改编小说为短剧
...
### 核心原则
1. **结构化改编** - 6个Skills形成完整改编流程
2. **用户掌控** - 每个关键节点需要用户确认
3. **Think Aloud** - 透明化决策过程
4. **短剧适配** - 专门针对短剧特点（节奏快、对白密集、时长短）
...
【判断任务类型】
   ↓
   ├─ A. 全新改编项目 → Skill 10 + Skill 11 + Skill 12 + Skill 13（所有集数）+ Skill 14 + Skill 15
   │
   └─ B. 继续改编某集 → Skill 13（单集场景大纲）+ Skill 14 + Skill 15
```

### 剧本改编 Skills
- **Skill 10**: 小说分析 - 深度分析小说，提取关键信息
- **Skill 11**: 结构分析 - 规划短剧的集数结构
- **Skill 12**: 人物改编 - 将小说人物改编为剧本人物
- **Skill 13**: 场景大纲 - 为每集设计详细的场景大纲
- **Skill 14**: 剧本创作 - 根据场景大纲，创作完整剧本
- **Skill 15**: 剧本审校 - 审校剧本，确保符合短剧拍摄要求
...
思考：
- 这是"全新改编项目"（任务类型 A）
- 需要执行完整的改编流程（Skill 10-15）
- 首先执行 Skill 10 分析小说

计划：
1. 执行 Skill 10：分析小说，生成《小说分析报告》
2. 执行 Skill 11：规划短剧结构（集数、章节映射）
3. 执行 Skill 12：改编人物方案
4. 等待用户确认后，继续 Skill 13-15

正在执行 Skill 10...
[读取小说设定、章节内容]
[生成《小说分析报告》]

《小说分析报告》已生成！保存在 adaptations/ 目录。
...
**当前版本**: 第二阶段（基础结构 + 剧本改编系统）

**已实现功能**:
- ✅ 目录结构
- ✅ CLAUDE.md 总纲
- ✅ 核心 Skills（1-9）
- ✅ 剧本改编 Skills（10-15）⭐ 新增
- ✅ 基础素材库框架
- ✅ 剧本改编目录结构 ⭐ 新增
